...
void OLED_Send(uint8_t *data, uint8_t len) {
  HAL_I2C_Master_Transmit(&hi2c1, OLED_I2C_ADDRESS, data, len, HAL_MAX_DELAY);
}
...
void OLED_Init() {
  ...
  OLED_SendCmd(0xD5); /* Display clock divide ratio / oscillator frequency. */
  OLED_SendCmd(0x80);

  OLED_SendCmd(0xA8); /* Multiplex ratio: 1/64. */
  OLED_SendCmd(0x3F);

  OLED_SendCmd(0xD3); /* Display offset. */
  OLED_SendCmd(0x00);
  OLED_SendCmd(0x40); /* Display start line = 0. */

  OLED_SendCmd(0x8D); /* SSD1306 charge pump. */
  OLED_SendCmd(0x14);

  OLED_SendCmd(0x20); /* Page addressing mode, matching OLED_ShowFrame(). */
  OLED_SendCmd(0x02);

  OLED_SendCmd(0xA1); /* Segment remap. */
  OLED_SendCmd(0xC8); /* COM scan direction remap. */

  OLED_SendCmd(0xDA); /* COM pins hardware configuration. */
  OLED_SendCmd(0x12);

  OLED_SendCmd(0x81); /* Contrast. */
  OLED_SendCmd(0xCF);

  OLED_SendCmd(0xD9); /* Pre-charge period for internal charge pump. */
  OLED_SendCmd(0xF1);

  OLED_SendCmd(0xDB); /* VCOMH deselect level. */
  OLED_SendCmd(0x40);

  OLED_SendCmd(0xA4); /* Resume display from RAM. */
